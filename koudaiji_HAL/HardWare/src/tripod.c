...
Tripod_Encoder_Struct Tripod_Encoder_X, Tripod_Encoder_Y; //云台编码器
...
void Tripod_Init(void)
{
    ...
    Tripod_Y_STEP.GPIOx = GPIOE;    Tripod_Y_STEP.GPIO_Pin = GPIO_PIN_8;   //E8 -> Y_STEP
    Tripod_Y_LIMIT.GPIOx = GPIOE;   Tripod_Y_LIMIT.GPIO_Pin = GPIO_PIN_12; //E12 -> Y限位信号读取
...
void Tripod_Y_Step(uint16_t num)
{
    ...
    __HAL_TIM_SET_PRESCALER(&htim3, 84*5);
...
void Tripod_Y_Go_Mechzero(void)
{
    ...
    if(Tripod_Y.is_limit == false)
    {
        ...
        HAL_TIM_Base_Stop(&htim3);
        __HAL_TIM_CLEAR_IT(&htim3, TIM_IT_UPDATE);
        __HAL_TIM_SET_PRESCALER(&htim3, 84);
    }
...
uint16_t Tripod_X_Speed_To_Prescaler(float speed)
{
    ...
    dis_coord = speed * 19.514f; //19.514 = 1366 / 70
    dis_angle = atan2(dis_coord, DIS_X) * 57.29577f;
    dis_step = (uint16_t)(dis_angle * X_STEP_ANGLE);
    per = 210000/dis_step; //210000 = 5000 * 84 / 2
...
uint16_t Tripod_Y_Speed_To_Prescaler(float speed)
{
    ...
    dis_coord = speed * 19.514f; //19.514 = 1366 / 70
    dis_angle = atan2(dis_coord, DIS_X) * 57.29577f;
    dis_step = (uint16_t)(dis_angle * Y_STEP_ANGLE);
    per = 210000/dis_step; //210000 = 5000 * 84 / 2
...
{
    int16_t encoder_bais;
    float angle_bais;
    int16_t coord;

    encoder_bais = __HAL_TIM_GET_COUNTER(&htim1) - 30000;
    angle_bais = (encoder_bais / 2000.0f * 360.0f) * (15.0f / 50.0f);
    coord = (tan(angle_bais / 57.29577f) * DIS_X) + 684;

    if(coord > 0)
    {
      Tripod_Encoder_X.coord = coord;  
    }
}
...
void Tripod_Y_Get_Encoder_Coord(void)
{
    ...
    encoder_bais = __HAL_TIM_GET_COUNTER(&htim4) - 30000;
    angle_bais = (encoder_bais / 2000.0f  * 360.0f) * (15.0f / 50.0f);
    line = hypot(fabs(Tripod_Encoder_X.coord - 684), DIS_X);
    coord = (tan(angle_bais / 57.29577f) * line) + Y_OFFSET;

    if(coord > 0)
    {
        Tripod_Encoder_Y.coord = coord;
    }
}
