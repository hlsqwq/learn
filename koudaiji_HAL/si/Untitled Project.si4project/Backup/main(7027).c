	...
	while(1){
		...
		if(Touch_Key_Value == TOUCH_KEY_C) //如果检测到按键“C”按下
		{
			...
			if(Touch_Key_Value == TOUCH_KEY_C) //延时两秒之后还能检测到按键“C”按下
			{
				Touch_Key_Value = TOUCH_KEY_NONE;
				Lcd_Clear(BLUE);
				Lcd_ShowString_HZ(0,0,(uint8_t *)"1.温度测量",WHITE);
				Lcd_ShowString_HZ(0,16,(uint8_t *)"2.项目功能",WHITE);
			}
		}

		else if(Touch_Key_Value == TOUCH_KEY_D) //如果检测到按键“C”按下
		{
			Touch_Key_Value = TOUCH_KEY_NONE;
			HAL_Delay(2000);
			if(Touch_Key_Value == TOUCH_KEY_D) //延时两秒之后还能检测到按键“C”按下
			{
				Touch_Key_Value = TOUCH_KEY_NONE;
				Lcd_Clear(BLUE);
				Lcd_ShowString_HZ(0,0,(uint8_t *)"温度测量与显示",WHITE);
				Lcd_ShowString_HZ(0,16,(uint8_t *)"当前温度:20.7 °C",WHITE);
				Lcd_ShowString_HZ(0,32,(uint8_t *)"按*键返回主界面",WHITE);
			}
		}

		else if(Touch_Key_Value == TOUCH_KEY_A) //如果检测到按键“C”按下
		{
			Touch_Key_Value = TOUCH_KEY_NONE;
			HAL_Delay(2000);
			
			if(Touch_Key_Value == TOUCH_KEY_A) //延时两秒之后还能检测到按键“C”按下
			{
				process_ac_sig=false;
				Lcd_Clear(BLUE);
				while (1)
				{
					Osci_Show();
