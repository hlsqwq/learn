...
int main(void)
{
  ...
  MX_USART6_UART_Init();
	...
	Lcd_Init();
	Lcd_Clear(BLUE);
	Lcd_ShowString_HZ(0,0,(uint8_t *)"江苏省技能大赛",WHITE);
	Lcd_ShowString_HZ(0,16,(uint8_t *)"智能电子设计与开发",WHITE);
	Lcd_ShowString_HZ(0,32,(uint8_t *)"工位号:333",WHITE);
	//HAL_UART_Receive_IT(&huart3,uart_buf,20);
	while(1)
	{
		HAL_UART_Transmit_IT(&huart5,"hello", strlen("hello"));
		HAL_Delay(1000);
	}
