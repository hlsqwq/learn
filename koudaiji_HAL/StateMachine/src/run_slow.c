...
void Run_Mode_Slow_Run(void)
{
    ...
    if(Run_Slow.first_flag == true)  //首次检测到该物体
    {
        ...
        if(Run_Slow.step == 3) //第四步，挨个指向物体
        {
            ...
            {
                Lcd_Show_Page_6();
                Lcd_Draw_Circle((uint16_t)(Run_Slow.res_coord[Run_Slow.i].x/4.27f), 240-(uint16_t)(Run_Slow.res_coord[Run_Slow.i].y/3.2f), 5, BLUE);
                ...
                Lcd_Show_Page_7();
                Object_Message_Store(Run_Slow.res_coord[Run_Slow.i].x, Run_Slow.res_coord[Run_Slow.i].y);
                Run_Slow.i++;
                Run_Slow.user_num1 = 0;
            }
    ...
    else
    {
        ...
        if(Run_Slow.step == 2) //第三步指向物体
        {
            ...
            {
                Lcd_Show_Page_6();
                Lcd_Draw_Circle((uint16_t)(Run_Slow.res_coord[Run_Slow.res_num-Run_Slow.need_point_num+Run_Slow.i].x/4.27f), 240-(uint16_t)(Run_Slow.res_coord[Run_Slow.res_num-Run_Slow.need_point_num+Run_Slow.i].y/3.2f), 5, BLUE);
                Relay_Control(RELAY_OFF); //指示之前关闭激光笔
                //屏幕上打印检测到的物体的信息
                ...
                Lcd_Show_Page_7();
                Object_Message_Store(Run_Slow.res_coord[Run_Slow.res_num-Run_Slow.need_point_num+Run_Slow.i].x, Run_Slow.res_coord[Run_Slow.res_num-Run_Slow.need_point_num+Run_Slow.i].y);
                Run_Slow.i++;
                Run_Slow.user_num1 = 0;
            }
