...
void Osci_Show(void)
{
    if(adc_get_flag == true) //ADC采样1024完毕
    {
		if(process_ac_sig){
			...
			adc_average=adc_average*3.3/4095;
			
			sprintf(arr,"avg voltage: %lf",adc_average);
			Lcd_ShowString_HZ(0,48,(uint8_t *)arr,WHITE);
			///计算实际电压
			//3300 /4095 mV * adc_average= 实际电压值; 
		}
    } 
...
void mysin(){
        ...
        Lcd_Osci_printf("Max:%1.2fV Min:%1.2fV Freq:%2.2fkHz   ", max_true, min_true, freq);  //打印波形参数数据
