...
def get_yolo_model(version: int, onnx_model_name: str):
    ...
    exported_filename = model.export(format="onnx",opset=18)  # export the model to ONNX format
...
def run_inference(onnx_model_file: Path):
    ...
    image = np.frombuffer(open('./1.png', 'rb').read(), dtype=np.uint8)
...
if __name__ == '__main__':
    ...
    version = 8
    onnx_model_name = Path(f"./export/yolov{version}n.onnx")
