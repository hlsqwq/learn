<template>
  <div class="pay">
    ...
    <div class="footer-fixed">
      ...
      <div class="tipsbtn" @click="orderSubmit">提交订单</div>
    </div>
  </div>
</template>
...
import { getOrder, orderSubmit } from '@/api/pay'
export default {
  name: 'PayIndex',
  data () {
    return {
  ...
  methods: {
    ...
    async getOrder () {
      if (this.mode === 'cart') {
        const { data: { order, personal } } = await getOrder(this.mode, { cartIds: this.cartIds.join(',') })
        this.order = order
    ...
    async orderSubmit () {
      ...
      await orderSubmit({
        ...
        mode: this.mode,
        remark: this.remark,
        cartIds: this.cartIds && this.cartIds.join(',')
      })
      this.$toast('支付成功')
      this.$router.push('/order', { dataType: 'all' })
    }
  },
  created () {
    this.getAddress()
