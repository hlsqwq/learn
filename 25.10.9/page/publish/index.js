...
btn.addEventListener('click', function () {
  ...
  if (document.querySelector("body > div.wrap > div.main > div.content > div > div.title > span")
    .innerHTML === '修改文章') {
    axios({
      ...
      myAlert(true, 'modify success')
      location.href = '../content/index.html'
    }).catch(err => {
      myAlert(false, 'modify fail')
...
!(() => {
  ...
  p.forEach((v, k) => {
    if (k === 'id') {
      document.querySelector("body > div.wrap > div.main > div.content > div > div.title > span").innerHTML = '修改文章'
      axios({
        url: `/v1_0/mp/articles/${v}`
      }).then(res => {
        console.log(res);
        const data = res.data
        document.querySelector("#title").value = data.title
        channel.children[0].remove()
        for (let i = 0; i < channel.children.length; i++) {
          const v = channel.children[i]
          if (+v.value === data.channel_id) {
            v.selected = true
          }
        }
        img.src = data.cover.images.length ? data.cover.images[0] : 'https://img2.baidu.com/it/u=2640406343,1419332367&fm=253&fmt=auto&app=138&f=JPEG?w=708&h=500'
        img.classList.add('show')
        document.querySelector('.place').classList.add('hide')
